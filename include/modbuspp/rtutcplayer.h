...
namespace Modbus {
  ...
  /**
   * @class RtuTcpLayer
   * @brief RTU-TCP/IP v4 & v6 network layer
   *
   * This class can not and should not be instantiated by the user.
   * It provides access to properties and methods specific to the RTU-TCP layer.
   *
  ...
  public:
       ...
       * @brief Constructor
       */
      RtuTcpLayer (const std::string & host, const std::string & service);
    ...
    protected:
      class Private;
      RtuTcpLayer (Private &dd);

    private:
      PIMP_DECLARE_PRIVATE (RtuTcpLayer)
  };
}
